...
"""
    ...
    comments_                = "Gui For hashLib"
    website_                 = "https://github.com/yucefsourani/PygiHashLibGui"
    translators_             = ("translator-credit")
    appname                  = "PygiHashLibGui"
    appwindowtitle           = "PygiHashLibGui"
    appid = "com.github.yucefsourani.PygiHashLibGui"
    if get_os()=="linux":
        ...
        if os.path.isfile("{}.png".format(appid)):
            icon_ = "{}.png".format(appid)
        else:
            icon_ = ""
    ...
    class AppWindow(Gtk.ApplicationWindow):
        def __init__(self, *args, **kwargs):
            ...
            self.props.title = "PyHashLibGui"
            sw=Gtk.ScrolledWindow()
            mainvbox = Gtk.VBox()
            mainvbox.set_margin_start(20)
            mainvbox.set_margin_end(20)

            
            hashlibbin = HashLibBin(self)
            
            mainvbox.add(hashlibbin)
            sw.add(mainvbox)
            self.add(sw)
            self.show_all()
            self.maximize()
